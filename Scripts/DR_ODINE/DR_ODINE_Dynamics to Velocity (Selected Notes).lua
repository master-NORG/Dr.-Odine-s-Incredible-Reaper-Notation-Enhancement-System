-- @description DR_ODINE_Dynamics to Velocity (Selected Notes)
-- @author DR_ODINE
-- @version 1.0
-- @provides [main] DR_ODINE_Dynamics to Velocity (Selected Notes).lua
-- @about
--   # Dynamics to Velocity (Selected Notes)
--   
--   Applies dynamics text events to selected MIDI note velocities only.
...
----------------------------------------------------------------
-- Apply dynamics and accents to selected note velocities only
----------------------------------------------------------------
local function apply_dynamics_and_accents(take, textsByPPQ, item, tr)
  local _, noteCount = reaper.MIDI_CountEvts(take)
  local changed = 0
  
  -- First, collect all selected note indices
  local selected_notes = {}
  for i = 0, noteCount-1 do
    local rv, sel = reaper.MIDI_GetNote(take, i)
    if rv and sel then
      selected_notes[i] = true
    end
  end
  
  -- If no notes are selected, skip processing
  local selected_count = 0
  for _ in pairs(selected_notes) do selected_count = selected_count + 1 end
  if selected_count == 0 then
    log("No selected notes found, skipping item")
    return 0
  end
  ...
  -- Apply dynamics to selected note velocities only (including accents)
  for i = 0, noteCount-1 do
    local rv, sel, mute, ppqStart, ppqEnd, chan, pitch, vel = reaper.MIDI_GetNote(take, i)
    if rv and selected_notes[i] then -- Only process selected notes
